...
const SPECIFIED_LANGUAGE_META_KEY =
    'de.joshuagleitze.markdown-it-prism.specifiedLanguage';
...
function highlight(
    ...
    return highlightWithSelectedLanguage(
        markdownit,
        options,
        text,
        selectLanguage(options, lang),
    );
}
 ...
 *  html-escaped.
 */
function highlightWithSelectedLanguage(
    markdownit: MarkdownIt,
    options: Options,
    text: string,
    [langToUse, prismLang]: SelectedPrismLanguage,
): string {
    return prismLang
        ...
        if (inlineToken.type === 'inline' && inlineToken.children !== null) {
            for (const [index, token] of inlineToken.children.entries()) {
                if (
                    ...
                    index + 1 < inlineToken.children.length
                ) {
                    extractInlineCodeSpecifiedLanguage(
                        token,
                        inlineToken.children[index + 1],
                    );
                }
            }
        }
 ...
 */
function extractInlineCodeSpecifiedLanguage(
    inlineCodeToken: Token,
    followingToken: Token,
) {
    const languageSpecificationMatch = followingToken.content.match(
        /^\{((?:[^\s}]+\s)*)language=([^\s}]+)((?:\s[^\s}]+)*)}/,
    );
    if (languageSpecificationMatch !== null) {
        ...
        followingToken.content = followingToken.content.slice(
            languageSpecificationMatch[0].length,
        );
        if (languageSpecificationMatch[1] || languageSpecificationMatch[3]) {
            followingToken.content = `{${languageSpecificationMatch[1] || ''}${(
                languageSpecificationMatch[3] || ' '
            ).slice(1)}}${followingToken.content}`;
        }
    }
}
...
function renderInlineCode(
    ...
    return (tokens, idx, renderOptions, env, self) => {
        ...
        const specifiedLanguage = inlineCodeToken.meta
            ? inlineCodeToken.meta[SPECIFIED_LANGUAGE_META_KEY] || ''
            : '';
        const [langToUse, prismLang] = selectLanguage(
            options,
            specifiedLanguage,
        );
        if (!langToUse) {
            ...
            const highlighted = highlightWithSelectedLanguage(
                markdownit,
                options,
                inlineCodeToken.content,
                [langToUse, prismLang],
            );
            inlineCodeToken.attrJoin(
                'class',
                languageClass(markdownit, langToUse),
            );
            return `<code${self.renderAttrs(
                inlineCodeToken,
            )}>${highlighted}</code>`;
        }
    };
...
export default function markdownItPrism(
    ...
    markdownit.renderer.rules.fence = applyCodeAttributes(
        markdownit,
        options,
        markdownit.renderer.rules.fence || renderFallback,
    );
    if (options.highlightInlineCode) {
        markdownit.core.ruler.after(
            'inline',
            'prism_inline_code_language',
            inlineCodeLanguageRule,
        );
        markdownit.renderer.rules.code_inline = renderInlineCode(
            markdownit,
            options,
            markdownit.renderer.rules.code_inline || renderFallback,
        );
    }
}
