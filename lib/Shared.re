...
let twitter =
    (~contentTitle, ~description, ~article_image, ~thumbnail, ~children, ()) => {
  Html.(
    <fragment>
      <meta
        name="twitter:card"
        content={article_image == None ? "summary" : "summary_large_image"}
      />
...
let pageWithTopAndBottom =
    (~css, ~backgroundImage=?, ~top, ~middle, ~bottom, ~children, ()) => {
  Html.(
    Css.(
      <body
        className={css([
...
let spacer = num =>
  Html.(<div style={"flex-basis: " ++ string_of_int(num) ++ "px"} />);
let hspace = num =>
  Html.(<div style={"height: " ++ string_of_int(num) ++ "px"} />);
let vspace = num =>
  Html.(<div style={"width: " ++ string_of_int(num) ++ "px"} />);
...
module Styles = {
  ...
  let colPaddingRules = [
    ...
    Media(
      "max-width: " ++ px(Consts.Media.skinnyFourCol),
      [("padding", px(Consts.colPadding / 2))],
    ),
    Media(
      "max-width: " ++ px(Consts.Media.oneCol),
      [("padding", px(Consts.colPaddingSmall))],
    ),
  ];

  let row = [A("display", "flex"), A("flex-direction", "row")];
  ...
  let subtleLink = [A("color", "currentColor"), ...hoverUnderline];
